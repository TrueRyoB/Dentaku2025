const testcase = [
    ...
    ["atan(-2, 2) - atan 1*-2, 1*(atantan1, 1)", { solution: "0",isValid: true }],
];
...
function parse(f) {
    ...
    //console.log("Parsed tokens => " + tokens.join(" "));
...
function tokenize(s) {
    let i = 0, n = s.length;
    const tokens = [];
    let funcCount = 0; //TODO: funcCountとskipCountを全部取っ払う
    let skipCount = 0; //TODO: 関数とカンマの直後に[を置くルールに則る　他はnum numの時
    let bStack = [];
    ...
    while (i < n) {
        ...
        if (isDigit(s[i])) {
            ...
            if (tokens.length > 0) {
                ...
                } else if (funcCount > 0 && (isNumber(peek) || isConstant(peek))){
                    tokens.push("]");
                    tokens.push("[");
                }
            }
        ...
        if (matchedFunc) {
            if (tokens.length > 0 && tokens[tokens.length-1] === "-" && (i === 1 || (!isNumber(tokens[tokens.length-2]) && !isConstant(tokens[tokens.length-2])))) {
                tokens.pop();
                tokens.push("-1");
                tokens.push("*");
            } 
        ...
        if (isConstant(s[i])) {
            ...
            if (tokens.length > 0) {
                const peek = tokens[tokens.length - 1];
                if (funcCount > 0 && (isNumber(peek) || isConstant(peek))) {
                    tokens.push("]");
                    tokens.push("[");
                }
            }
    ...
    for(let i =0; i < funcCount; ++i) tokens.push("]");
    return tokens;
}
